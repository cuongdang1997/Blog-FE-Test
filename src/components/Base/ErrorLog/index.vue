<template>
    <div v-if="logs.length > 0" @click="visible = true">
        ...
        <el-dialog class="dialog" title="Error log" :visible.sync="visible" width="80%" :append-to-body="true">
            <el-table :data="logs" border :header-cell-style="{'text-align': 'center'}">
                <el-table-column label="Scope index" align="center" width="50">
                    <template slot-scope="scope">
                        {{scope.$index + 1}}
                    </template>
                </el-table-column>
                <el-table-column label="Scope time" align="center" width="150">
                    <template slot-scope="scope">
                        {{scope.row.time | formatTime('{y}-{m}-{d} {h}:{i}')}}
                    </template>
                </el-table-column>
                <el-table-column label="bug" min-width="350">
                    <template slot-scope="scope">
